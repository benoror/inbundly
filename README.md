...
* Optionally color bundles to match their Gmail label color — either a subtle
  ...
  (enable in Options)
* Select or archive all of a bundle's messages from its row: a select-all
  checkbox (Gmail's toolbar actions then apply to the selection) and an
  archive-all button, both on hover and while the bundle is open
